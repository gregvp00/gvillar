...
export const snippets = {
  appPreview: `import AppBottomDrawer from "@/components/AppBottomDrawer";
...
export async function getHighlightedSnippets() {
  const theme = "vitesse-dark";
  const lang = "tsx";

  const [appPreviewHtml, appNavHtml, appBottomDrawerHtml] = await Promise.all([
    codeToHtml(snippets.appPreview, { lang, theme }),
    codeToHtml(snippets.appNav, { lang, theme }),
    codeToHtml(snippets.appBottomDrawer, { lang, theme }),
  ]);
